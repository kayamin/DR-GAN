...
def Is_D_strong(real_output, syn_output, id_label_tensor, pose_label_tensor, illum_label_tensor, Nd, Np,  thresh=0.9):
    """
    # Discriminator の正解率を算出し， 指定した以上の正解率であれば， 十分強いとみなす

    """
    _, id_real_ans = torch.max(real_output[:, :Nd], 1)
    _, pose_real_ans = torch.max(real_output[:, Nd+1:Nd+1+Np], 1)
    _, illum_real_ans = torch.max(real_output[:, Nd+1+Np:], 1)
    ...
    illum_real_precision = (illum_real_ans==illum_label_tensor).type(torch.FloatTensor).sum() / real_output.size()[0]
